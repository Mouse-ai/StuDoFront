...
import { User, LogOut, ChevronDown, ArrowRight, Mail, Lock, LayoutDashboard, GraduationCap } from 'lucide-react';
			...
			<div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[90%] max-w-4xl">
				<div className="flex items-center justify-between px-5 py-3 bg-white/80 backdrop-blur-md border border-gray-200/60 rounded-2xl shadow-lg shadow-indigo-100/50">
					...
					<div ref={containerRef} className="relative">
						<button onClick={() => setShowTooltip(!showTooltip)} className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl transition text-sm font-medium shadow-md shadow-indigo-200">
							<User size={16} /> Войти
						</button>

						<div className={tooltipClasses}>
							<div className="grid transition-[grid-template-rows] duration-300 ease-[cubic-bezier(0.4,0,0.2,1)]" style={{ gridTemplateRows: showTooltip ? '1fr' : '0fr' }}>
	...
	return (
		<div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[90%] max-w-4xl">
			<div className="flex items-center justify-between px-5 py-3 bg-white/80 backdrop-blur-md border border-gray-200/60 rounded-2xl shadow-lg shadow-indigo-100/50">
				...
				<div ref={containerRef} className="relative">
					<button onClick={() => setShowTooltip(!showTooltip)} className="flex items-center gap-2 px-3 py-2 hover:bg-gray-100 rounded-xl transition">
...
function AuthForm({ activeTab, onSubmit }: { activeTab: 'login' | 'register'; onSubmit: any }) {
	...
	return (
		<form onSubmit={handleSubmit} className="space-y-3">
			...
			<button type="button" disabled className="w-full py-2.5 bg-gray-100 text-gray-500 rounded-xl text-sm font-medium flex items-center justify-center gap-2 cursor-not-allowed border border-gray-200">
				<GraduationCap size={14} /> Войти с помощью ИСУ
			</button>
		</form>
	);
}